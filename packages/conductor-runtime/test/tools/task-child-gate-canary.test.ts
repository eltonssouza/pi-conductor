...
it(
	"a destructive bash call is denied by the SAME extension construction createGovernedChildSessionSpawner uses for a delegation child's own gate, with hasUI:false (no UI ever bound to a child session) -- identical deny/reason/approvalMethod to the top-level 'no UI' case (permission-gate.task.test.ts)",
	async () => {
...
it(
	"the SAME child-gate construction denies a CRITICAL-tier command unconditionally, before any approval branch even runs -- proving classification itself (not only the approval step) is fully re-established for a delegation child",
	async () => {
		const auditPath = join(workspace.root, ".conductor", "audit.jsonl");
		const auditTrailWriter = createAuditTrailWriter(auditPath);
		const extension = createPermissionGateExtension({
